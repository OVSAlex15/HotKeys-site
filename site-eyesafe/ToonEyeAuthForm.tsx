...
interface ToonEyeAuthFormProps {
  type: "signin" | "signup";
  onSubmit: (values: any) => void;
}

const ToonEyeAuthForm: React.FC<ToonEyeAuthFormProps> = ({ type, onSubmit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Show arm animation on submit
    if (armLeftRef.current && armRightRef.current) {
      // Show arms
      gsap.set([armLeftRef.current, armRightRef.current], { visibility: 'visible' });
        ...
        onComplete: () => {
          gsap.to(armRightRef.current, {
            ...
            onComplete: () => {
              ...
              gsap.set([armLeftRef.current, armRightRef.current], { visibility: 'hidden' });
  ...
  useEffect(() => {
    // Setup eye following for email field
    const handleEmailFocus = () => {
      const inputRect = emailInputRef.current?.getBoundingClientRect();
      if (inputRect && eyeLeftRef.current && eyeRightRef.current) {
        // Look down at email field
        gsap.to([eyeLeftRef.current, eyeRightRef.current], { y: 3, duration: 0.3 });
        
        // Happy mouth
        if (mouthRef.current) {
          gsap.to(mouthRef.current, {
            attr: { d: "M100.2,101c-0.4,0-1.4,0-1.8,0c-2.7-0.3-5.3-1.1-8-2.5c-0.7-0.3-0.9-1.2-0.6-1.8 c0.2-0.5,0.7-0.7,1.2-0.7c0.2,0,0.5,0.1,0.6,0.2c3,1.5,5.8,2.3,8.6,2.3s5.7-0.7,8.6-2.3c0.2-0.1,0.4-0.2,0.6-0.2 c0.5,0,1,0.3,1.2,0.7c0.4,0.7,0.1,1.5-0.6,1.9c-2.6,1.4-5.3,2.2-7.9,2.5C101.7,101,100.5,101,100.2,101z" },
            duration: 0.3
          });
        }
      }
    ...
    const handleEmailBlur = () => {
      if (eyeLeftRef.current && eyeRightRef.current) {
        // Look forward
        gsap.to([eyeLeftRef.current, eyeRightRef.current], { y: 0, duration: 0.3 });
      }
    };
    ...
    const handleEmailInput = (e: Event) => {
      ...
      const eyeX = (inputProgress * 2 - 1) * 3; // Limit the range
      
      if (eyeLeftRef.current && eyeRightRef.current) {
        gsap.to([eyeLeftRef.current, eyeRightRef.current], { x: eyeX, duration: 0.1 });
      }
    };
    
    // Setup eye following for password field
    const handlePasswordFocus = () => {
      const inputRect = passwordInputRef.current?.getBoundingClientRect();
      if (inputRect && eyeLeftRef.current && eyeRightRef.current) {
        // Look down at password field
        gsap.to([eyeLeftRef.current, eyeRightRef.current], { y: 6, duration: 0.3 });
        
        // Concerned mouth for password
        if (mouthRef.current) {
          gsap.to(mouthRef.current, {
            attr: { d: "M100.2,101c-0.4,0-1.4,0-1.8,0c-2.7-0.3-5.3-1.1-8-2.5c-0.7-0.3-0.9-1.2-0.6-1.8 c0.2-0.5,0.7-0.7,1.2-0.7c0.2,0,0.5,0.1,0.6,0.2c3,1.5,5.8,2.3,8.6,2.3s5.7-0.7,8.6-2.3c0.2-0.1,0.4-0.2,0.6-0.2 c0.5,0,1,0.3,1.2,0.7c0.4,0.7,0.1,1.5-0.6,1.9c-2.6,1.4-5.3,2.2-7.9,2.5C101.7,101,100.5,101,100.2,101z" },
            duration: 0.3
    ...
    if (emailInput) {
      ...
      emailInput.addEventListener('click', handleEmailInput);
    }
    ...
    if (passwordInput) {
      ...
      passwordInput.addEventListener('click', handlePasswordInput);
    }
    ...
    return () => {
      if (emailInput) {
        ...
        emailInput.removeEventListener('click', handleEmailInput);
      }
      ...
      if (passwordInput) {
        ...
        passwordInput.removeEventListener('click', handlePasswordInput);
      }
    };
  }, []);
  ...
  return (
    <div className="bg-white p-8 rounded-xl shadow-md border border-blue-100">
      <div className="relative w-full max-w-[200px] h-[200px] mx-auto mb-8">
        <svg 
          ref={svgRef}
        ...
        >
          ...
          <g className="eyeL" ref={eyeLeftRef}>
            <circle cx="85.5" cy="78.5" r="3.5" fill="#3a5e77" />
            <circle cx="84" cy="76" r="1" fill="#fff" />
          </g>
          <g className="eyeR" ref={eyeRightRef}>
          ...
          <g className="arms" clipPath="url(#armMask)">
            <g className="armL" ref={armLeftRef} style={{ visibility: 'hidden' }}>
            ...
            <g className="armR" ref={armRightRef} style={{ visibility: 'hidden' }}>
      ...
      <h1 className="text-2xl font-bold mb-6 text-blue-600 text-center">
        {type === 'signin' ? 'Войти в аккаунт' : 'Зарегистрироваться'}
      </h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="space-y-2">
          ...
          <Input
            ...
            required
            className="w-full h-12 text-lg bg-blue-50 border-blue-200 focus:border-blue-400"
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <Input
              ...
              required
              className="w-full h-12 text-lg pr-10 bg-blue-50 border-blue-200 focus:border-blue-400"
            />
            <button
              type="button"
...
export default ToonEyeAuthForm;
